...
#
#   <reportsDir>/verification.json   .claude\scripts\run-checks.ps1 だけが書く
#   <reportsDir>/test-report.md      同上（レポート）
#   <reportsDir>/test-spec.md        同上（テスト仕様書）
#   <reportsDir>/review-record.json  .claude\scripts\record-review.ps1 だけが書く
#
# reportsDir は .claude\harness.json で決める（既定 reports）。
#
...
#
# 正規の生成経路（run-checks.ps1 / record-review.ps1）はコマンド行に
...
# 設定はプロジェクトルートから読む。毎回 git を呼ばずに済むよう、Claude Code が渡す
# CLAUDE_PROJECT_DIR を優先する。
$projectDir = $env:CLAUDE_PROJECT_DIR
if ([string]::IsNullOrWhiteSpace($projectDir)) { $projectDir = [string](Get-Prop $hookInput 'cwd' '') }
$reportsDir = Get-ReportsDir (Get-HarnessConfig $projectDir)

# 守る対象。リポジトリからの相対パス。
#
# test-report.md と test-spec.md（テスト仕様書）は人間が「何を検証したか」を確かめるための
# 文書なので、記録と同じく生成スクリプト以外に書かせない。
$ProtectedFiles = @(
    @{ Path = "$reportsDir/verification.json"; Producer = '.\.claude\scripts\run-checks.ps1' },
    @{ Path = "$reportsDir/test-report.md"; Producer = '.\.claude\scripts\run-checks.ps1' },
    @{ Path = "$reportsDir/test-spec.md"; Producer = '.\.claude\scripts\run-checks.ps1' },
    @{ Path = "$reportsDir/review-record.json"; Producer = '.\.claude\scripts\record-review.ps1 -Reviewers <名前> -Verdict pass' }
)
...
書き込みとみなして拒否します（node -e での読み取りなど）。
...
if (-not [string]::IsNullOrWhiteSpace($filePath)) {
    ...
    # 記録を作る側のスクリプトと、ゲートの有無を決める harness.json も、
    # 書き換えればゲートを無力化できる。
    if ($normalizedPath -match '/\.claude/(hooks/|scripts/|settings\.json|harness\.json)') {
...
foreach ($line in ($prepared -split '\r?\n')) {
    ...
    #
    # 文で割らないと `.\.claude\scripts\run-checks.ps1; Set-Content <記録>` を通してしまい、
