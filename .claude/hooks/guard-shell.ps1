...
#
#   - 保護ブランチ（既定 main / master）への直接コミット
#   - 保護ブランチへの push
...
# push 先として指定された参照が保護ブランチ（harness.json の protectedBranches）を指しているか。
#
...
function Test-ProtectedRef {
    ...
    return Test-ProtectedBranch -Branch $destination -Config $Config
}
...
$protectedList = (Get-ProtectedBranches $config) -join " / "
...
"@
            ...
            if ($arguments -contains '--delete') {
                foreach ($value in $positional) {
                    if (Test-ProtectedRef $value $config) {
                        Deny-Tool "リモートの保護ブランチ（$protectedList）は削除できません。"
                    }
                }
            }

            # 明示的に保護ブランチを push しようとしている（refspec も含む）。
            foreach ($value in ($positional | Select-Object -Skip 1)) {
                if (-not (Test-ProtectedRef $value $config)) { continue }

                # 作られた直後の空のリモートへの初回 push。守る履歴が無いので通す。
                if (Test-RemoteIsEmpty -GitExe $gitExe -RepoRoot $repoRoot) { continue }

                Deny-Tool @"
保護ブランチ（$protectedList）への直接 push は禁止されています（AGENTS.md の規約）。
...
"@
            ...
            if ($positional.Count -le 1 -and (Test-ProtectedBranch $branch $config) -and
...
作業用ブランチに切り替えてから push してください:
    ...
    git push -u origin $branchExample
"@
            }
        }
        ...
        'commit' {
            if (Test-ProtectedBranch $branch $config) {
                Deny-Tool @"
$branch ブランチへの直接コミットは禁止されています（AGENTS.md の規約）。
...
"@
        ...
        'branch' {
            if ($arguments -contains '-D' -or $arguments -contains '-d') {
                foreach ($value in $positional) {
                    if (Test-ProtectedRef $value $config) {
                        Deny-Tool "保護ブランチ（$protectedList）は削除できません。"
                    }
                }
            }
        }
