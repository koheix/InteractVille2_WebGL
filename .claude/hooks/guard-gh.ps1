...
#
#   - <reportsDir>/verification.json  いまの HEAD で検証が全緑（.claude\scripts\run-checks.ps1 が書く）
#   - <reportsDir>/review-record.json いまの HEAD をレビュー済み（.claude\scripts\record-review.ps1 が書く）
#
# どちらを必須にするかと reportsDir は .claude\harness.json の prGate / reportsDir で決める。
# 設定が無い・壊れているときは両方必須に倒す（fail-closed）。harness.json の変更は
# guard-records.ps1 が「ガード設定の変更」として通知するので、黙って緩めることはできない。
#
...
# ゲートが有効か。JSON の false（真偽値）と明示されたときだけ無効にする。
# 文字列 "false" などの書き損じは有効のまま（fail-closed）。
function Test-GateEnabled {
    param($Config, [string]$Path)

    $value = Get-Prop $Config $Path $true
...
foreach ($invocation in $invocations) {
    ...
    if ($subcommand -eq 'pr' -and $action -eq 'merge') {
        Deny-Tool @"
PR のマージは人間が行います（AGENTS.md の必須ルール）。
...
"@
    ...
    $reportsDir = Get-ReportsDir $config
    $requireVerification = Test-GateEnabled $config 'prGate.requireVerification'
    $requireReview = Test-GateEnabled $config 'prGate.requireReview'

    $runChecks = '.\.claude\scripts\run-checks.ps1'
    $recordReview = '.\.claude\scripts\record-review.ps1 -Reviewers test-auditor,pr-reviewer -Verdict pass'

    if ($requireVerification) {
        $verification = Read-JsonFile (Join-Path $repoRoot "$reportsDir\verification.json")
        if ($null -eq $verification) {
            Deny-Tool @"
検証結果が記録されていないため PR を作れません。
...
"@
        ...
        if ($verifiedCommit -ne $headSha) {
            Deny-Tool @"
検証結果が今のコミットのものではありません。

  記録: $verifiedCommit
  HEAD: $headSha

コミットのあとに $runChecks を実行し直してください。
"@
        }
        ...
        if ($failed -ne 0) {
            Deny-Tool @"
検証が $failed 件失敗しています（記録が壊れている場合も失敗として扱います）。
落ちたまま PR を作らないでください。

原因を直してから $runChecks を実行し直してください。
...
"@
    ...
    if (-not $requireReview) { continue }

    $review = Read-JsonFile (Join-Path $repoRoot "$reportsDir\review-record.json")
    if ($null -eq $review) {
        Deny-Tool @"
