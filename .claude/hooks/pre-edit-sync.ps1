...
#   3. 保護ブランチ（harness.json の protectedBranches）を直接編集しようとしたら拒否する
#   4. origin を fetch し、upstream より遅れていれば pull --ff-only する
#      作業ツリーが汚れていて fast-forward できないときは拒否して指示を出す
#   5. origin の既定ブランチがブランチの分岐点より進んでいれば rebase を促す（拒否はしない）
...
# 同期の間引き間隔（秒）。環境変数 > harness.json の syncThrottleSeconds > 既定 600 秒。
# 環境変数はテストから上書きするためのもの。
function Get-ThrottleSeconds {
    param($Config)

    $parsed = 0
    if ($env:CLAUDE_HARNESS_SYNC_THROTTLE_SECONDS -and
        [int]::TryParse($env:CLAUDE_HARNESS_SYNC_THROTTLE_SECONDS, [ref]$parsed)) { return $parsed }
    if ([int]::TryParse([string](Get-Prop $Config 'syncThrottleSeconds' ''), [ref]$parsed)) { return $parsed }
    return 600
}
...
$throttleSeconds = Get-ThrottleSeconds $config

# 追跡対象外の作業ファイルへの書き込みでは同期しない（無駄な fetch を避ける）。
if ($filePath) {
    $normalized = $filePath -replace '/', '\'
    $reportsPattern = '*\' + ((Get-ReportsDir $config) -replace '/', '\') + '\*'
    if ($normalized -like '*\.claude\state\*' -or $normalized -like $reportsPattern) { exit 0 }
}

$branch = Get-CurrentBranch -GitExe $gitExe -RepoRoot $repoRoot
if (-not $branch) { exit 0 }

if (Test-ProtectedBranch $branch $config) {
    $branchExample = Get-BranchExample $config
    Deny-Tool @"
$branch ブランチを直接編集することはできません（AGENTS.md の規約）。
...
# 分岐元（origin の既定ブランチ）が進んでいないか。進んでいれば rebase を促す（作業は止めない）。
$defaultRemoteBranch = Get-DefaultRemoteBranch -GitExe $gitExe -RepoRoot $repoRoot -Config $config
