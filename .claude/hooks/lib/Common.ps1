# フック・ハーネススクリプト共通ユーティリティ。
#
# 各フックスクリプトと .claude\scripts\ から dot-source して使う。
# プロジェクト固有の値はここに書かず、.claude\harness.json から Get-HarnessConfig で読む。
...
# .claude\harness.json（プロジェクト固有の設定）を読む。
#
# 無い・壊れている場合は空オブジェクトを返し、各設定は呼び出し側の既定値に倒れる。
# 既定値はゲートを「有効」にする側に倒してあるので、設定を壊しても守りは緩まない。
function Get-HarnessConfig {
    param([string]$RepoRoot)

    if ([string]::IsNullOrWhiteSpace($RepoRoot)) { return [pscustomobject]@{} }
    $path = Join-Path $RepoRoot '.claude\harness.json'
...
# 保護対象ブランチの一覧。harness.json の protectedBranches、無ければ main / master。
function Get-ProtectedBranches {
    param($Config)
    $branches = @(Get-Prop $Config 'protectedBranches' @('main', 'master') | Where-Object { $_ })
    if ($branches.Count -eq 0) { return @('main', 'master') }
    return $branches
}

# 保護対象ブランチか。
function Test-ProtectedBranch {
    param([string]$Branch, $Config)
    return @(Get-ProtectedBranches $Config) -contains $Branch
}

# 作業ブランチ名の例（拒否メッセージで案内する）。
function Get-BranchExample {
    param($Config)
    return [string](Get-Prop $Config 'branchExample' 'feature/<内容がわかる名前>')
}

# 検証記録（verification.json / review-record.json / test-report.md）を置くディレクトリ。
# リポジトリからの相対パスを / 区切りで返す。
function Get-ReportsDir {
    param($Config)
    $dir = [string](Get-Prop $Config 'reportsDir' 'reports')
    $dir = ($dir -replace '\\', '/').Trim('/')
    if ([string]::IsNullOrWhiteSpace($dir)) { return 'reports' }
    return $dir
}

# PR の土台になる origin 側の既定ブランチ（origin/main など）。見つからなければ $null。
function Get-DefaultRemoteBranch {
    ...
    $configured = [string](Get-Prop $Config 'defaultBranch' '')
    $candidates = @()
    if ($configured) { $candidates += "origin/$configured" }
    ...
    foreach ($candidate in ($candidates | Select-Object -Unique)) {
        ...
        if ($verify.Ok -and $verify.StdOut) { return $candidate }
    }
    return $null
}
